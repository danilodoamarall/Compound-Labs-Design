...
export interface CommandPaletteProps {
  ...
  contentDelay?: number;
  currentPath?: string;
  /** Ajuste local: rótulos que o componente trazia fixos em inglês e que o
      hub precisa traduzir, porque o site é bilíngue. */
  a11yTitle?: string;
  a11yDescription?: string;
  closeLabel?: string;
  /** Contagem anunciada a quem usa leitor de tela. String com "%n" no lugar do
      número, ou função, quando o idioma precisa de concordância de plural. */
...
function CommandPaletteView({
  ...
  a11yDescription = "Search commands and navigate results with the keyboard.",
      ...
      <DialogPrimitive.Root onOpenChange={setOpen} open={open}>
        <DialogPrimitive.Portal>
          ...
          >
            ...
            {/* Ajuste local: nem este componente nem o cmdk anunciam quantos
                resultados sobraram. Sem isto, quem usa leitor de tela ouve o
                primeiro item e nunca sabe o tamanho da lista. */}
            <div aria-live="polite" className="sr-only" role="status">
              {hasQuery
                ? typeof resultsLabel === "function"
                  ? resultsLabel(flatItems.length)
                  : resultsLabel.replace("%n", String(flatItems.length))
                : ""}
            </div>
                ...
                {isSearching ? (
                  ...
                  indexedSections.map((section, sectionIndex) => (
                    <React.Fragment key={section.heading}>
                      {sectionIndex > 0 ? (
                        <div aria-hidden className="mx-2 my-2 h-px bg-border/60" />
                      ) : null}
                      {/* Ajuste local: um listbox só aceita option e group como
                          filhos. Sem o role aqui, os invólucros de seção ficam
                          entre o listbox e as options, e o leitor de tela pode
                          perder a contagem ou pular itens. */}
                      <div aria-label={section.heading} className="px-2 py-1" role="group">
                        <div
                          aria-hidden
                          className="px-1 pb-1.5 font-medium text-[11px] text-muted-foreground uppercase tracking-[0.16em]"
                        >
                          {section.heading}
                        </div>
