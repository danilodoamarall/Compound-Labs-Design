...
/** Cards dos artigos, no formato medido na referência para os cards de pessoa:
    379×443, raio 16, fundo rgba(0,0,0,0.4), borda 0.67px, três colunas com 24
    de gap, rodapé de 72px.

    Onde a referência põe foto, entra a capa do artigo: o número em mono, o par
      ...
      {articles.map((a, i) => {
        ...
        return (
          <li key={a.slug}>
            ...
            >
              <span aria-hidden className="relative aspect-[379/300] overflow-hidden">
                <span
                ...
                <span className="absolute inset-0 flex flex-col justify-between p-5">
                  ...
                  <span className="font-mono text-[76px] font-medium leading-none tracking-[-0.06em] text-white/90">
                    {n}
                  </span>
                </span>
              </span>

              <span className="flex flex-1 flex-col gap-1 border-t border-[var(--stage-line)] p-5">
                <span className="text-[16px] font-medium leading-snug text-[#EDEDED]">{a.title}</span>
...
/** Faixa gigante atrás do título, como o "Hello. Hallo. Hola." da referência:
    100px, peso 600, tracking -3px, opacidade 0.4. Estática, como no original. */
export function StageMarquee({ words }: { words: string[] }) {
  return (
    <span
      aria-hidden
      className="pointer-events-none absolute -top-2 left-0 hidden w-full select-none overflow-hidden whitespace-nowrap font-semibold leading-none tracking-[-3px] opacity-40 md:block"
      style={{
        fontSize: 100,
        background: "linear-gradient(to right, #FAFAFA, rgba(250,250,250,0.05) 78%)",
